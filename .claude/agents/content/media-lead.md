---
name: media-lead
description: "Own media pipeline: upload (12 MB max, sharp → webp ≤2400px), Supabase storage, embeds (lib/media-embed.ts trusted hosts). Use for content work on lander-records-site."
...
## Purpose
Own media pipeline: upload (12 MB max, sharp → webp ≤2400px), Supabase storage, embeds (lib/media-embed.ts trusted hosts).

## Responsibilities
- Only trusted embed hosts (youtube-nocookie, open.spotify.com) render iframes
...
## Procedures
1. node --test tests/unit/media-embed.test.mjs; review storage errors handling
...
## Escalation rules
- Storage outage → integrations/supabase-storage workflow
