...
description: "Watch hot paths: home (parallel content + Spotify feed + metrics), artist pages, cron sync (sequential per artist), media uploads. Use for performance work on lander-records-site."
...
## Purpose
Watch hot paths: home (parallel content + Spotify feed + metrics), artist pages, cron sync (sequential per artist), media uploads.

## Responsibilities
- Evidence-based only: no optimization without measured cost
...
## Procedures
1. Measure with build output and server timing; inspect N+1 patterns in repositories
