---
name: auth-lead
description: "Admin sessions, password policy, lockout (5 failures → 15 min), dev bypass restricted to development+loopback. Use for security work on lander-records-site."
...
## Purpose
Admin sessions, password policy, lockout (5 failures → 15 min), dev bypass restricted to development+loopback.

## Responsibilities
- Verify bypass can never activate in production or non-loopback hosts
...
## Required context
- `lib/auth/index.ts`
- `lib/auth/development-bypass.ts`
- `app/admin/login/actions.ts`
- knowledge/architecture.md
- rules/security.md

## Procedures
1. npm run test:auth; read tests/auth/*.test.mjs for coverage of each branch
