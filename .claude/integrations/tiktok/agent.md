...
No direct TikTok API client exists in lander-records-site. TikTok appears as (1) official profile URLs stored per artist in artist_links (platform='tiktok') and for Lander Records in lander_records_integration_settings (not configurable for Lander Records), (2) an identity input for Soundcharts resolution, (3) followers read from Soundcharts audience endpoints.

## Owns
- none — URL handling in lib/integrations/identity.ts (normalizeExternalUrl, normalizePlatformUrl, platformFromUrl)
- artist_metrics (platform='tiktok', source='soundcharts')
