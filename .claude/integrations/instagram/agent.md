# Instagram integration agent

Mode: `via-soundcharts`

No direct Instagram API client exists in lander-records-site. Instagram appears as (1) official profile URLs stored per artist in artist_links (platform='instagram') and for Lander Records in lander_records_integration_settings (instagram_url), (2) an identity input for Soundcharts resolution, (3) followers read from Soundcharts audience endpoints.

## Owns
- none — URL handling in lib/integrations/identity.ts (normalizeExternalUrl, normalizePlatformUrl, platformFromUrl)
- artist_metrics (platform='instagram', source='soundcharts')
