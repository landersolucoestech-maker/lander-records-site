...
import {
  INTERNAL_NAVIGATION_DESTINATIONS,
  menuLabel,
...
function NavigationForm({ item, items, canDelete, onClose }: { item: NavigationSummary | null; items: NavigationSummary[]; canDelete: boolean; onClose: () => void }) {
  ...
  return <section aria-label={item ? `Editar ${item.label}` : "Novo item de menu"} className={styles.editor}>
    <div className={styles.editorHeading}>
      <div><p>{item ? "EDIÇÃO DE ITEM" : "NOVO ITEM"}</p><h2>{item ? item.label : "Adicionar item de navegação"}</h2><span>Configure somente estrutura, destino, ordem e visibilidade do menu.</span></div>
      <button aria-label="Fechar editor" className={styles.iconButton} onClick={onClose} type="button"><AdminIcon name="x" /></button>
    </div>
...
export default function NavigationManager({ allItems, canDelete = false, canEdit = true, initialFilters = {}, items, message, metrics: metricCounts, preview = false }: { allItems?: NavigationSummary[]; canDelete?: boolean; canEdit?: boolean; initialFilters?: Filters; items: NavigationSummary[]; message?: { kind: "error" | "success"; text: string } | null; metrics?: Metrics; preview?: boolean }) {
  ...
  const [menu, setMenu] = useState(initialFilters.menu || "all");
  const [hierarchy, setHierarchy] = useState(initialFilters.hierarchy || "all");
  ...
  useEffect(() => {
    ...
    if (menu !== "all") params.set("menu", menu);
    if (hierarchy !== "all") params.set("hierarchy", hierarchy);
    const timer = window.setTimeout(() => router.replace(`${pathname}${params.size ? `?${params}` : ""}`, { scroll: false }), 180);
    return () => window.clearTimeout(timer);
  }, [hierarchy, menu, pathname, preview, query, router, status, type]);

  const filtered = useMemo(() => {
    if (!preview) return items;
    const needle = query.trim().toLocaleLowerCase("pt-BR");
    return items.filter((item) => (!needle || `${item.label} ${item.url}`.toLocaleLowerCase("pt-BR").includes(needle)) && (status === "all" || (status === "active" ? item.enabled : !item.enabled)) && (type === "all" || item.linkType === type) && (menu === "all" || item.menuKey === menu) && (hierarchy === "all" || (hierarchy === "root" ? !item.parentId : Boolean(item.parentId))));
  }, [hierarchy, items, menu, preview, query, status, type]);
  ...
  const metrics: Array<[IconName, string, number, string]> = [["navigation", "Total de itens", counts.total, "Registros de navegação"], ["home", "Ativos", counts.active, "Habilitados no site público"], ["x", "Inativos", counts.inactive, "Ocultos no site público"], ["external", "Links externos", counts.external, "Destinos fora do site"]];
  ...
  return <div className={styles.manager} data-testid="navigation-manager">
    {preview ? <div className="adminPreviewNotice">BACKEND_ENVIRONMENT_DEFERRED · dados de navegação isolados, sem alterações ou persistência.</div> : null}
    ...
    <section className={styles.catalog}>
      <div className={styles.toolbar} role="search">
        <label className={styles.search}><span className="srOnly">Buscar itens de navegação</span><AdminIcon name="search" size={17} /><input onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por rótulo ou URL..." type="search" value={query} /></label>
        <label><span className="srOnly">Status</span><select aria-label="Filtrar por status" onChange={(event) => setStatus(event.target.value)} value={status}><option value="all">Todos os status</option><option value="active">Ativos</option><option value="inactive">Inativos</option></select></label>
        <label><span className="srOnly">Tipo</span><select aria-label="Filtrar por tipo" onChange={(event) => setType(event.target.value)} value={type}><option value="all">Todos os tipos</option><option value="internal">Internos</option><option value="external">Externos</option></select></label>
        <label><span className="srOnly">Menu</span><select aria-label="Filtrar por menu" onChange={(event) => setMenu(event.target.value)} value={menu}><option value="all">Todos os menus</option><option value="primary">Principal</option><option value="footer">Rodapé</option></select></label>
        <label><span className="srOnly">Hierarquia</span><select aria-label="Filtrar por hierarquia" onChange={(event) => setHierarchy(event.target.value)} value={hierarchy}><option value="all">Toda a hierarquia</option><option value="root">Itens principais</option><option value="child">Subitens</option></select></label>
        {hasFilters && filtered.length ? <button className="adminButton" onClick={clearFilters} type="button">Limpar filtros</button> : null}
        {canEdit && !preview ? <button className="adminButton primary" onClick={() => setEditor("new")} type="button"><AdminIcon name="plus" size={14} />Novo item</button> : null}
      </div>

      {filtered.length ? <div aria-label="Itens de navegação cadastrados" role="table">
        <div className={styles.tableHeader} role="row">{["Item do menu", "Tipo", "URL / destino", "Abre em", "Status", "Ordem", "Ações"].map((label) => <span key={label} role="columnheader">{label}</span>)}</div>
        <div className={styles.rows} role="rowgroup">{filtered.map((item, index) => <div className={styles.rowWrap} key={item.id}>
          {index === 0 || filtered[index - 1].menuKey !== item.menuKey ? <div className={styles.menuHeading}>{menuLabel(item.menuKey)}</div> : null}
          <div className={`${styles.row} ${item.parentId ? styles.child : ""}`} data-testid="navigation-row" role="row">
            ...
            <div aria-label={`Ações de ${item.label}`} className={styles.actions} role="cell">
              {canEdit && !preview ? <button aria-label={`Editar ${item.label}`} onClick={() => setEditor(item)} type="button"><AdminIcon name="pages" size={16} /></button> : preview ? <button aria-label={`Editar ${item.label} indisponível no preview`} disabled type="button"><AdminIcon name="pages" size={16} /></button> : null}
              {!preview && item.safeDestination ? item.linkType === "external" ? <a aria-label={`Abrir destino de ${item.label} (abre em nova aba)`} href={item.url} rel="noopener noreferrer" target="_blank"><AdminIcon name="external" size={16} /></a> : <Link aria-label={`Abrir destino de ${item.label}`} href={item.url}><AdminIcon name="external" size={16} /></Link> : <button aria-label={`Abrir destino de ${item.label} indisponível${preview ? " no preview" : " por URL inválida"}`} disabled type="button"><AdminIcon name="external" size={16} /></button>}
            </div>
          </div>
        </div>)}</div>
        <div className={styles.resultCount}>Mostrando {filtered.length} de {counts.total} itens</div>
    ...
    <section aria-label="Limites e boas práticas de navegação" className={styles.guidance}>
      <article><AdminIcon name="navigation" /><div><h2>Hierarquia administrativa</h2><p>O modelo registra pai e filho, mas Cabeçalho, menu mobile e Rodapé exibem atualmente somente itens principais.</p></div></article>
      <article><AdminIcon name="pages" /><div><h2>Páginas e menus são independentes</h2><p>Criar uma página não adiciona automaticamente um link, e remover um item não exclui conteúdo.</p></div></article>
      <article><AdminIcon name="settings" /><div><h2>Boas práticas</h2><p>Use rótulos curtos, confirme o destino público e mantenha as posições organizadas dentro de cada menu.</p></div></article>
    </section>
  </div>;
}
