...
export default async function PagesAdminPage() {
  ...
  const [rows, sections] = await Promise.all([
    ...
    }).from(pages).orderBy(asc(pages.title)),
    db.select({
      id: pageSections.id,
      ...
      subtitle: pageSections.subtitle,
    }).from(pageSections).orderBy(asc(pageSections.position)),
  ]);
  ...
  const summary: PageSummary[] = rows.map((page) => {
    ...
    return {
      ...
      sectionCount: pageStructure.length,
      enabledSectionCount: pageStructure.filter((section) => section.enabled).length,
