...
const fallbackDescriptions: Record<string, string> = {
  hero: "Seção principal da página.", intro: "Apresentação e conteúdo introdutório.", shortcuts: "Atalhos principais da página.", artists: "Conteúdo de artistas relacionado à página.", releases: "Conteúdo de lançamentos relacionado à página.", news: "Publicações e notícias relacionadas à página.", history: "Conteúdo institucional e histórico.", identity: "Identidade, missão, visão e valores.", pillars: "Pilares e frentes de atuação.", companies: "Empresas e frentes do ecossistema.", methodology: "Metodologia e processo de trabalho.", artist_filters: "Filtros disponíveis para o catálogo de artistas.", artist_list: "Lista de artistas exibida ao público.", news_categories: "Filtros editoriais e categorias de notícias.", news_list: "Lista de publicações exibida ao público.",
};
...
export default function PageManager({ canEdit = true, demoMode = false, pages, preview = false }: { canEdit?: boolean; demoMode?: boolean; pages: PageSummary[]; preview?: boolean }) {
  ...
  return <div className={styles.manager} data-testid="pages-manager">
    <section className={styles.selectionCard} aria-label="Página selecionada">
      ...
      <label className={styles.pageSelector}><span>Página</span><select aria-label="Selecionar página" onChange={(event) => { setSelectedId(event.target.value); setDeleteOpen(false); }} value={selected.id}>{effectivePages.map((page) => <option key={page.id} value={page.id}>{displayTitle(page)} · {pageKind(page)} · {page.enabled ? "publicada" : "não publicada"}</option>)}</select></label>
      <div className={styles.pageActions}>
        ...
        {canEdit && !demoMode && !preview ? <button className={`${styles.outlineButton} ${styles.dangerButton}`} onClick={() => setDeleteOpen(true)} type="button"><AdminIcon name="trash" size={17} /><span>Excluir</span></button> : <button aria-disabled="true" className={`${styles.outlineButton} ${styles.dangerButton}`} type="button"><AdminIcon name="trash" size={17} /><span>Excluir</span></button>}
      </div>
    </section>

    <section className={styles.structureCard} aria-label="Estrutura da página">
      <div className={styles.structureHeading}><div><span className={styles.eyebrow}>Estrutura da página</span><strong>{title}</strong><small>{sections.length} {sections.length === 1 ? "seção" : "seções"} · {kind}</small></div><Link className={styles.primaryButton} href={editHref}><span aria-hidden="true">+</span> Criar seção</Link></div>
      <div className={styles.table} role="table" aria-label={`Seções de ${title}`}>
        <div className={styles.tableHeader} role="row"><span role="columnheader">Seção</span><span role="columnheader">Status</span><span role="columnheader">Ações</span></div>
        <div role="rowgroup">{sections.map((section, index) => <div className={styles.sectionRow} key={section.id} role="row">
          <div className={styles.sectionIdentity} role="cell"><span className={styles.position}>{String(index + 1).padStart(2, "0")}</span><span><strong>{sectionTitle(section)}</strong><small>{sectionDescription(section)}</small></span></div>
          ...
          <div className={styles.sectionActions} role="cell"><Link className={styles.configureButton} href={editHref}><AdminIcon name="sliders" size={17} /><span>Configurar</span></Link><Link aria-label={`Mais ações para ${sectionTitle(section)}`} className={styles.moreButton} href={editHref}><AdminIcon name="more" size={18} /></Link></div>
        </div>)}{!sections.length ? <div className={styles.noSections}>Nenhuma seção cadastrada nesta página.</div> : null}</div>
      </div>
    </section>
