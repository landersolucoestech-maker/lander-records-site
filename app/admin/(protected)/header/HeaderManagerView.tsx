...
export function HeaderManagerView({ data, preview = false, viewer = false }: { data: HeaderManagerData; preview?: boolean; viewer?: boolean }) {
  ...
  const metrics: Array<[IconName, string, string, string]> = [
    ["media", "Logo do cabeçalho", data.globalLogoUrl ? "Configurada no CMS" : "Fallback estrutural", data.publicLogoSrc],
  ...
  return <div className={styles.manager} data-testid="header-manager">
    ...
    {viewer ? <div className="adminNotice">Este módulo está em modo somente leitura para a sessão atual.</div> : null}
    ...
    <div className={styles.workspace}>
      <section aria-label="Fontes do cabeçalho" className={styles.sources}>
        ...
        <article className={styles.sourceBlock}><div><h3>Logo pública</h3><p>Cabeçalho e rodapé consomem a logo selecionada em Configurações, com fallback para a marca estrutural da Lander Records.</p></div><div className={styles.logoPreview}><Image alt={data.brandName} height={46} src={data.publicLogoSrc} unoptimized width={170} /></div><dl><div><dt>Asset utilizado</dt><dd>{data.publicLogoSrc}</dd></div><div><dt>Logo global do CMS</dt><dd>{data.globalLogoUrl ? "Configurada e em uso no site público" : "Sem mídia selecionada; fallback estrutural em uso"}</dd></div></dl><Link className="adminButton" href={moduleHref("/admin/settings")}>Abrir configurações globais</Link></article>
        <article className={styles.sourceBlock}><div><h3>Chamada principal (CTA)</h3><p>Desktop e mobile repetem o mesmo conteúdo estrutural.</p></div><dl><div><dt>Texto</dt><dd>{data.ctaLabel}</dd></div><div><dt>Destino</dt><dd>{data.ctaUrl}</dd></div><div><dt>Persistência</dt><dd>Não existe no modelo atual</dd></div></dl><p className={styles.deferred}>A configuração do CTA ainda não possui persistência no CMS.</p></article>
