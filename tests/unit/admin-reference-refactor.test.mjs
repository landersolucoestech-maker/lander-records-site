...
const mediaKitPreviewStyles = read("app/admin/(protected)/media-kit/MediaKitPreview.module.css");
...
test("settings internal tabs use scoped mutations and preserve RBAC", () => {
  ...
  assert.match(users, /const mockMode = mockDataEnabled\(\)/);
  assert.match(users, /!mockMode && \(session\.source !== "session" \|\| session\.user\.role !== "owner"\)/);
  assert.match(users, /disabled=\{!canManage\}/);
